...
import usePlayPieces from "./hooks/usePlayPieces";
...
const playPieces = [
  {
    id: 1,
    shadowColor: "hsl(230, 89%, 55%)",
    background: "linear-gradient(hsl(230, 89%, 62%), hsl(230, 89%, 65%))",
    icon:
      <svg width="49" height="59">
  ...
  {
    id: 2,
    shadowColor: "hsl(40, 84%, 43%)",
    background: "linear-gradient(hsl(39, 89%, 49%), hsl(40, 84%, 53%))",
    icon:
      <svg width="51" height="58">
...
export default function Game() {
  ...
  const { isSelected, playPiece, renderPlayPieces } = usePlayPieces(
    (current) => {
      const randomIndex = Math.floor(Math.random() * playPieces.length);
      console.log("CURRENT PLAYPIECE =", current);
      console.log("RANDOM PLAYPIECE =", playPieces[randomIndex]);
    },
    playPieces
  );
